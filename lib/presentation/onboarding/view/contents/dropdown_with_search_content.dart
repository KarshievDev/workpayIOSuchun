...
class _SearchableDropdownContentState extends State<SearchableDropdownContent> {
  ...
  @override
  void didUpdateWidget(SearchableDropdownContent oldWidget) {
    super.didUpdateWidget(oldWidget);
    // Update selected company when widget properties change
    if (widget.selectedItem != oldWidget.selectedItem) {
      selectedCompany = widget.selectedItem;
    }
    // Update filtered items when items list changes
    if (widget.items != oldWidget.items) {
      filteredItems = widget.items;
      // If no company is selected and items are now available, select the first one
      if (selectedCompany == null && widget.items.isNotEmpty) {
        selectedCompany = widget.items.first;
