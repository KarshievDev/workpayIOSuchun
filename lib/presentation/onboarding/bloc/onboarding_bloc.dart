...
class OnboardingBloc extends HydratedBloc<OnboardingEvent, OnboardingState> {
  ...
  FutureOr<void> _onCompanyLoaded(CompanyListEvent event, Emitter<OnboardingState> emit) async {
    ...
    if (kDebugMode) {
      print('OnboardingBloc: Loading company list...');
    }
    final companyModel = await _metaClubApiClient.getCompanyList();
    companyModel.fold((l) {
      if (kDebugMode) {
        print('OnboardingBloc: Failed to load company list - $l');
      }
      emit(state.copyWith(status: NetworkStatus.failure));
    }, (r) {
      if (kDebugMode) {
        print('OnboardingBloc: Company list loaded successfully');
        print('Company count: ${r?.companyList?.length ?? 0}');
      }
