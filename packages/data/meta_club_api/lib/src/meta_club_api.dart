...
class MetaClubApiClient {
  ...
  Future<Either<Failure, CompanyListModel?>> getCompanyList() async {
    ...
    try {
      if (kDebugMode) {
        print('========== COMPANY LIST API CALL ==========');
        print('URL: $api');
        print('Method: GET');
      }

      final response = await httpService.getRequest(api);
      ...
      if (response == null) {
        ...
        return Left(ExceptionFailure(exception: Exception('Response is null')));
      }

      if (response.statusCode != 200) {
        if (kDebugMode) {
          print('ERROR: Status code is not 200: ${response.statusCode}');
        }
        return Left(GeneralFailure.httpStatus(
            response.statusCode ?? 0, 'GET', response.statusMessage ?? 'Unknown error'));
      }
      ...
      if (response.data == null) {
        ...
        return Left(ExceptionFailure(exception: Exception('Response body is null')));
      }

      final companyListModel = CompanyListModel.fromJson(response.data);
      if (kDebugMode) {
        print('Parsed company count: ${companyListModel.companyList?.length ?? 0}');
        print('========== END COMPANY LIST API CALL ==========');
      }

      return Right(companyListModel);
      ...
      return Left(ExceptionFailure(exception: Exception('TypeError: $e')));
    } on Exception catch (e) {
      if (kDebugMode) {
